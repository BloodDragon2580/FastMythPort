...
local function buildFavoritesButtons(parent)
	...
	for i = 1, #core.favoritesDB.collection do
		...
		-- ✅ WICHTIG: Buttons im Favoriten-Ring müssen ebenfalls als "favorite" markiert sein,
		-- damit man sie per Linksklick wieder entfernen kann (Handler sitzt in buttons.lua).
		btn.favorite = true
...
local function FavoritesTooltipHandler(self)
	...
	GameTooltip:AddLine(L["Hold Left-Click to drag"])
	GameTooltip:AddLine(L["Right-Click to toggle"])
	GameTooltip:AddLine(' ')
	GameTooltip:AddLine(L["Selected Favorites: "] .. #core.favoritesDB.collection)
...
local function BuildFavorites()
	-- ✅ Unique global name (keine Frame-Name-Kollisionen)
	local btn = CreateFrame('BUTTON', 'FastMythPortFavoritesButton', UIParent, 'UIPanelButtonTemplate')
	...
	btn.items = CreateFrame('Frame', 'FastMythPortFavoritesItems', btn)
